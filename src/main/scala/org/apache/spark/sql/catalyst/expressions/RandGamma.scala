...
case class RandGamma(child: Expression, shape: Expression, scale: Expression, hideSeed: Boolean = false) extends TernaryExpression
  ...
  @transient protected lazy val scaleVal: Double = shape.dataType match {
    case IntegerType => shape.eval().asInstanceOf[Int]
    case LongType => shape.eval().asInstanceOf[Long]
    case FloatType | DoubleType => shape.eval().asInstanceOf[Double]
  }
  ...
  def this(child: Expression) = this(child, Literal(1.0, DoubleType), Literal(1.0, DoubleType), false)
...
//  }
  ...
  override protected def withNewChildrenInternal(newFirst: Expression, newSecond: Expression, newThird: Expression): Expression = copy(child = newFirst, shape = newSecond, scale = newThird)
}
